...
impl DynamicContent {
	pub async fn init() -> Result<DynamicContent, InitContentErr> {
		Ok(DynamicContent { ramhorns: Ramhorns::from_folder("www/content/templates")?, posts: PostCollection::from_folder("www/content/posts").await? })
	}
...
pub fn create_routes<'a>(app: Arc<App>) -> BoxedFilter<(impl Reply + 'a,)> {
	...
	let post_index_route = warp::path!("posts").and(with_app(app.clone())).and_then(handle_post_index);

	let post_route = warp::path!("posts" / String).and(with_app(app.clone())).and_then(handle_post);

	let tag_index_route = warp::path!("tags").and(with_app(app.clone())).and_then(handle_tag_index);

	let tag_route = warp::path!("tags" / String).and(with_app(app.clone())).and_then(handle_tag);

	warp::get()
		.and(static_pages.or(landing_route).or(discord_route).or(post_index_route).or(post_route).or(tag_index_route).or(tag_route))
		.recover(move |rej| recover(rej, app.clone()))
		.with(warp::log("quatweb::routes"))
		.boxed()
}
...
async fn recover(rej: Rejection, app: Arc<App>) -> Result<impl Reply, Infallible> {
	...
	let rendered = template.render(&TemplatingContext { errno: code.into(), message, include_error_css: true, settings: &app.settings });
...
async fn handle_landing(app: Arc<App>) -> Result<impl Reply, Rejection> {
	...
	let templating_context =
		TemplatingContext { posts: &content.posts.all_posts.iter().take(5).filter(|post| !post.meta.draft).collect::<Vec<_>>(), settings: &app.settings };
...
async fn handle_discord(app: Arc<App>) -> Result<impl Reply, Rejection> {
	...
	let templating_context = TemplatingContext { settings: &app.settings };
...
async fn handle_post_index(app: Arc<App>) -> Result<impl Reply, Rejection> {
	...
	let templating_context = TemplatingContext { posts, count, many: count > 1, settings: &app.settings };
...
async fn handle_tag(tag: String, app: Arc<App>) -> Result<impl Reply, Rejection> {
	...
	let templating_context = TemplatingContext { posts: &tagged_posts, count, many: count > 1, tag: &tag, settings: &app.settings };
...
async fn handle_tag_index(app: Arc<App>) -> Result<impl Reply, Rejection> {
	...
	let templating_context = TemplatingContext { tags, count, many: count > 1, settings: &app.settings };
